...
import GeodowVerticalNavExpansionPanel from "./GeodowVerticalNavExpansionPanel";
...
class GeodowVerticalNav extends Component {
    state = {
      collapsed: true
    };
    ...
    renderLevels = data => {
      return data.map((item, index) => {
        if (item.children) {
          return (
            <GeodowVerticalNavExpansionPanel item={item} key={index}>
              {this.renderLevels(item.children)}
            </GeodowVerticalNavExpansionPanel>
          );
        } else {
          return (
  ...
  export default withStyles(styles)(GeodowVerticalNav);